...
public:
  ...
  enum distances_enum {
    dist_error, partial_rmsd, rmsd, tm_score, rmsd_native_diff, euclidean, euclidean_loop, euclidean_diff_abs, euclidean_partial_mario, euclidean_mario, mario_first_last
  };
  ...
  void init_distances_map() {
    ...
    distances_map["tm_score"] = tm_score;
  ...
  void
  dump_popul(std::string tag, const std::vector<Individual>& popul);
...
class CalculateTMscoreDistancePopulation : public CalculateDistancePopulation
{
public:
  CalculateTMscoreDistancePopulation() : CalculateDistancePopulation() {

  }

  CalculateTMscoreDistancePopulation(const core::pose::PoseOP& p , FitFunctionPtr sfxn,  std::string ss_in, core::scoring::ScoreFunctionOP pscore, double radius) : CalculateDistancePopulation(p, sfxn, ss_in, pscore, radius) {}

  double
  current_distance_calculation(core::pose::Pose& pose_1, core::pose::Pose& pose_2) override;
};
...
public:
  ...
  PrintRMSDvsMarioAnalisys( std::string p, CalculateDistancePopulationPtr calculate_partial_rmsd_, CalculateDistancePopulationPtr calculate_partial_euc_  ) : prot_name(p), calculate_partial_rmsd(calculate_partial_rmsd_), calculate_partial_euc(calculate_partial_euc_) {
    reso = 0;
  }

  std::string extra_tags = "";
  void set_extra_tags(std::string tags_)  {
    extra_tags = tags_;
  }

  int reso;
  void set_resolution(int r_) {
    reso = r_;
  }

  void dump_pdb(std::vector<Individual> popul) {
    std::vector<core::pose::PoseOP> popul_pdb;
    ...
    for (int i = 0; i < popul.size(); i++) {
      std::string pose_name = "cart_pose_"+prot_name+"_"+id+"_"+std::to_string(i) + "_.pdb";
      std::cout << "ind " << i << " : " << pose_name << std::endl;
      popul_pdb[i]->dump_pdb(pose_name);
    }
    std::cout << "END POSE LIST" << std::endl;
  }
