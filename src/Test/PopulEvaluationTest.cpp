...
#include "PopulEvaluationTest.hpp"
#include <vector>

PopulEvaluationTest::PopulEvaluationTest(boost::shared_ptr<DE_Operator> app_operator_in) {
...
PopulEvaluationTest::start_popul() {
...
void
PopulEvaluationTest::init_operator_for_test() {
...
bool
PopulEvaluationTest::run() {
  std::vector<Individual> test_population = current_population;
  current_population = mpi_calculator->run(current_population, 1);

  for (int i = 0; i < test_population.size(); i++) {
    if (test_population[i].score != current_population[i].score) {
      std::cout << test_population[i].score << " != " << current_population[i].score << std::endl;
