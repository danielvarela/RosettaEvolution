...
#include "../Movers/FitFunction.hpp"
#include "../Movers/InitPopulation.hpp"
#include "../Movers/PrintBestIndividual.hpp"
...
class MPIfastCrowdingMoverDE : public MPICrowdingMoverDE
{
public:
  boost::shared_ptr<LocalSearchIndividualMover> local_search;
  std::vector<int> nearest_inds;

  MPIfastCrowdingMoverDE(ConfigurationDE pt, FitFunctionPtr scfxn_in, std::vector<Individual> initial_population,  boost::shared_ptr<LocalSearchIndividualMover> local_search_in) : MPICrowdingMoverDE(pt, scfxn_in, initial_population, local_search_in) {
    //    mpi_calculator = boost::shared_ptr<MasterRosettaCalculator>(new MasterEvaluateAndNearestCalculator(scfxn_in, calculate_distances_popul));
  }

  MPIfastCrowdingMoverDE(boost::property_tree::ptree options, FitFunctionPtr scfxn_in, std::vector<Individual> initial_population,  boost::shared_ptr<LocalSearchIndividualMover> local_search_in) : MPICrowdingMoverDE(options, scfxn_in, initial_population, local_search_in) {
    // mpi_calculator = boost::shared_ptr<MasterRosettaCalculator>(new MasterEvaluateAndNearestCalculator(scfxn_in, calculate_distances_popul));
  }

  void apply() override;

  bool select_population(const std::vector<Individual>& trial_popul) override;
...
public:
  ...
  bool select_population(const std::vector<Individual>& trial_popul) override {
    MoverDE::select_population(trial_popul);
  }


  virtual std::vector<std::vector<Individual> > create_seeds(std::vector<Individual> popul, std::vector<core::pose::PoseOP> popul_pdb);
  void apply();
};


class MPISeedsSantosMoverDE : public MPISeedsMoverDE
{
public:
  MPISeedsSantosMoverDE(ConfigurationDE pt, FitFunctionPtr scfxn_in, std::vector<Individual> initial_population,  boost::shared_ptr<LocalSearchIndividualMover> local_search_in) : MPISeedsMoverDE(pt, scfxn_in, initial_population, local_search_in) {
  }

  MPISeedsSantosMoverDE(boost::property_tree::ptree options, FitFunctionPtr scfxn_in, std::vector<Individual> initial_population,  boost::shared_ptr<LocalSearchIndividualMover> local_search_in) : MPISeedsMoverDE(options, scfxn_in, initial_population, local_search_in) {
  }

  void
  build_population_per_seed(std::vector<Individual> popul, std::vector<int> current_seed, std::vector<int> processed_inds, int m_size, int max_subpopuls, std::map<int, std::vector<int> >& population_per_seed );

  void print_current_subpopuls(std::vector<Individual> popul, std::vector<int> current_seed,  std::vector<std::vector<Individual> > subpopuls);

  std::vector<std::vector<Individual> > create_seeds(std::vector<Individual> popul, std::vector<core::pose::PoseOP> popul_pdb) override;
};


class MPISeedsSantosKmeansMoverDE : public MPISeedsSantosMoverDE
{
public:
  MPISeedsSantosKmeansMoverDE(ConfigurationDE pt, FitFunctionPtr scfxn_in, std::vector<Individual> initial_population,  boost::shared_ptr<LocalSearchIndividualMover> local_search_in) : MPISeedsSantosMoverDE(pt, scfxn_in, initial_population, local_search_in) {
  }

  MPISeedsSantosKmeansMoverDE(boost::property_tree::ptree options, FitFunctionPtr scfxn_in, std::vector<Individual> initial_population,  boost::shared_ptr<LocalSearchIndividualMover> local_search_in) : MPISeedsSantosMoverDE(options, scfxn_in, initial_population, local_search_in) {
  }

  std::vector<int>
  build_kmeans_seeds(std::vector<Individual> popul, std::vector<int> current_seed, std::vector<int> processed_inds, int m_size, int max_subpopuls, std::map<int, std::vector<int> > population_per_seed );
